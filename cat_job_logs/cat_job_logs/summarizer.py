...
from cat_job_logs import logger
...
class CollectionSummarizer(Summarizer):
    def __init__(self, collection_id, **kwargs):
        super(CollectionSummarizer, self).__init__(
            cat_job_logs.reader.CollectionReader(collection_id), **kwargs)
...
class JobSummarizer(Summarizer):
    def __init__(self, job, **kwargs):
        ...
        if self.job.get('log'):
            try:
                rdr = cat_job_logs.reader.CollectionReader(self.job['log'])
                ...
                label = self.job['uuid']
        if rdr is None:
            rdr = cat_job_logs.reader.LiveLogReader(self.job['uuid'])
